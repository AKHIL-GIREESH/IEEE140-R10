...
const SignUpButtons = ({ signUpData, verifyMail, setAuth, verifyPassword }) => {
  ...
  const signedUser = useMutation({
    mutationKey: ["signUp"],
    mutationFn: () =>
      fetch("http://159.65.7.52:5000/api/auth/register/", {
        method: "POST",
  ...
  console.log(
    Object.values(signUpData).includes("") && !verifyMail && !verifyPassword
  ); //&& !verifyCaptcha)
    ...
    >
      ...
      <div
        style={{
          height: "50%",
          marginTop: "2%",
          width: window.innerHeight > window.innerWidth ? "60vw" : null,
        }}
      >
        <button
        ...
        >
          {signedUser.isPending ? "Loading" : "SIGNUP"}
        </button>
      </div>
